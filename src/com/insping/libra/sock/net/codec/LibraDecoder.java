    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        try {
            ...
            switch (LibraMessageType.search(head.getType())) {
                case REGIST_REQ: {
                    //逻辑服务器注册到网关服务器
                    message.setBody(ServerRegistData.parseFrom(result));
                    break;
                }
                case HEARTBEAT_REQ: {
                    //逻辑服务器定时的心跳请求
                    message.setBody(HeartbeatData.parseFrom(result));
                    break;
                }
                case MESSAGE_RESP: {
                    // 普通消息请求和回复(不作处理)
                    message.setBody(ResHttpMessageData.parseFrom(result));
                    break;
                }

                default:
                    LibraLog.info("libraMessage Type is default. type = " + head.getType());
                    break;
            }
            out.add(message);
        } catch (Exception e) {
